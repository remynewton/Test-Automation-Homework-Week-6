...
# After Mentor Suggestions and Further Work

I then refactored the above code to use collection streamings instead (which was a bit of a headache for me).

My new code snippet in Main.java for this:
```
Thread thread1 = new Thread(criminal1);
        Thread thread2 = new Thread(officer1);
        thread1.start();
...
An example snippet of code from Criminal.java:
```
public byte position;
private volatile boolean isRunning = true;
...
I also added a whole new class called ConnectionPool but I haven't implemented it in my Main method yet:
```
package com.laba.solvd.hw;
